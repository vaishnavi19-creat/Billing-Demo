...
export class InvoiceValidator {
    static validateGetAllInvoices() {
        return [
            ...
            query('pageNumber').optional().isInt({ min: 1 }).withMessage('Page number must be a positive integer'),
        ];
    }

    static validateCreateInvoice() {
        return [
    ...
    static validateInvoiceId() {
        return [
            query('id').notEmpty().withMessage('Invoice ID is required').isString(),
        ];
    }
}
