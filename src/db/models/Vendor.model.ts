...
//     public async addVendor(vendorData: Vendor): Promise<VendorReq> {
//         try {
//             console.log('Jumped in VendorModel => addVendor()');
//             const savedVendor = await this.repository.save(vendorData);
//             return savedVendor; 
...
//     public async getAllVendors(limit: number = 10, pageNumber: number = 1): Promise<Vendor[]> {
//         try {
//             console.log('Jumped in VendorModel => getAllVendors()');
//             const skip = (limit * pageNumber) - limit;
//             return await this.repository
//                 .createQueryBuilder('vendor')
//                 .leftJoinAndSelect('vendor.user', 'user') // Assuming vendor has a user relation
//                 .skip(skip)
//                 .take(limit)
//                 .getMany();
//         } catch (error) {
...
//     public async softDeleteVendor(id: number): Promise<DeleteResult> {
//         try {
//             console.log('Jumped in VendorModel => softDeleteVendor()');
//             return await this.repository.update(id, { vendorStatus: false }); // Adjust this based on your soft delete implementation
//         } catch (error) {
//             throw new Error(error);
//         }
//     }

//     public async updateVendor(vendorId: number, updatedData: Partial<Vendor>): Promise<Vendor | null> {
//         try {
//             console.log('Jumped in CVendorModel => updateVendor()');
//             const vendor = await this.repository.findOne({ where: { id: vendorId } });
//             if (!vendor) {
//                 return null; 
//             }
